...
<template>
  <section class="sticky top-16 z-20 bg-white/90 dark:bg-black/80 backdrop-blur-xl border-y border-black/10 dark:border-white/10">
    <nav aria-label="Sections" class="max-w-7xl mx-auto px-5 sm:px-8 py-2 sm:py-3 flex items-center gap-1.5 sm:gap-2 overflow-x-auto scrollbar-none">
      <template v-for="c in items" :key="c.id">
        <a
          v-if="!asFilter"
          :href="`#${c.id}`"
          class="shrink-0 inline-flex items-center gap-1.5 sm:gap-2 h-8 sm:h-10 px-3 sm:px-4 rounded-full text-xs sm:text-sm font-semibold transition-all bg-gray-100 text-gray-700 hover:bg-gray-200 dark:bg-white/[0.06] dark:text-gray-300 dark:hover:bg-white/[0.1]"
        >
        ...
        <button
          ...
          class="shrink-0 inline-flex items-center gap-1.5 sm:gap-2 h-8 sm:h-10 px-3 sm:px-4 rounded-full text-xs sm:text-sm font-semibold transition-all"
          :class="active === c.id

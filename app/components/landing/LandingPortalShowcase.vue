    ...
    <div class="relative max-w-7xl mx-auto px-5 sm:px-8">
      ...
      <div class="grid grid-cols-1 sm:grid-cols-3 gap-8 sm:gap-12 mb-12 max-w-3xl">
        <div>
          ...
          <p class="mt-1 text-[10px] text-gray-500 uppercase tracking-[0.2em]">
            {{ copy.ui.appRatingLabel }}
          </p>
        </div>
        <div>
          <UIcon
            name="i-lucide-zap"
            class="size-5 text-secondary-500 mb-1.5"
          />
          <p class="text-4xl font-black leading-none">
            &lt; 1s
          </p>
          <p class="mt-1 text-[10px] text-gray-500 uppercase tracking-[0.2em]">
            {{ copy.ui.timeToBookLabel }}
          </p>
        </div>
        <div>
          <UIcon
            name="i-lucide-wifi-off"
            class="size-5 text-emerald-500 mb-1.5"
          />
          <p class="text-4xl font-black leading-none">
            {{ copy.ui.offlineLabel }}
          </p>
          <p class="mt-1 text-[10px] text-gray-500 uppercase tracking-[0.2em]">
            {{ copy.ui.pwaReadyLabel }}
          </p>
        </div>
      </div>

      <NuxtLink
        to="/portal/features#portal"
      ...
      <ul class="border-t border-black/10 dark:border-white/10 max-w-2xl">
        <li
          v-for="(f, i) in copy.portal.bullets"
          :key="f"
        ...
        >
          ...
          <span
            aria-hidden="true"
            class="h-px w-4 bg-black/20 dark:bg-white/20 group-hover:w-10 group-hover:bg-secondary-500 transition-all duration-300"
          />
          <span class="text-sm sm:text-base font-medium text-gray-700 dark:text-gray-300">{{ f }}</span>
        </li>
      </ul>

      <!-- ═══ Tenant showcase — browser-frame row ═══ -->
      <div class="mt-20 pt-16 border-t border-black/10 dark:border-white/10">
        ...
        <div class="flex items-end justify-between gap-4 mb-12 flex-wrap">
          <div>
            <LandingSectionEyebrow :label="copy.ui.threeTenantsLabel" />
            <h3 class="mt-3 text-2xl sm:text-3xl font-black tracking-tight">
              {{ copy.ui.oneSourceThreeBrands }}
            </h3>
          </div>
          <p class="text-sm text-gray-500 max-w-xs">
            {{ copy.ui.threeBrandsTagline }}
          </p>
        </div>
        ...
        <div class="flex lg:grid lg:grid-cols-3 gap-6 lg:gap-8 overflow-x-auto lg:overflow-visible scrollbar-none snap-x lg:snap-none pb-4 lg:pb-0 -mx-5 px-5 lg:mx-0 lg:px-0">
          ...
          >
            <!-- Browser window -->
            <div class="relative">
              <!-- Brand color wash behind window -->
              <div
                aria-hidden="true"
                class="absolute -inset-4 bg-gradient-to-br opacity-20 blur-3xl transition-opacity duration-500 group-hover:opacity-35"
                :class="t.grad"
              />

              <div class="relative rounded-2xl overflow-hidden bg-white dark:bg-[#0c0c0c] ring-1 ring-black/10 dark:ring-white/10 shadow-2xl shadow-black/15 transition-transform duration-500 group-hover:-translate-y-1.5">
                <!-- Browser chrome (Safari-style) -->
                <div class="h-9 px-3 flex items-center gap-2 bg-gray-50 dark:bg-white/[0.04] border-b border-black/5 dark:border-white/10">
                ...
                <div class="aspect-[16/10] relative">
                  <!-- Soft brand wash inside viewport -->
                  <div
                    aria-hidden="true"
                    class="absolute inset-0 bg-gradient-to-br opacity-[0.08]"
                    :class="t.grad"
                  />

                  <!-- App layout: left rail + main -->
                  <div class="relative h-full flex">
                    <!-- Left rail -->
                    <div class="w-10 sm:w-12 shrink-0 border-e border-black/5 dark:border-white/10 flex flex-col items-center py-3 gap-3">
                      <div
                        class="size-6 rounded-md bg-gradient-to-br flex items-center justify-center text-white font-black text-[9px]"
                        :class="t.grad"
                      >
                        {{ t.logo }}
                      </div>
                      <div class="w-full flex flex-col items-center gap-1.5 mt-1">
                        <span
                          ...
                          :key="n"
                          aria-hidden="true"
                          class="size-1.5 rounded-full bg-black/15 dark:bg-white/15"
                        />
                      </div>
                    </div>

                    <!-- Main column -->
                    <div class="flex-1 min-w-0 p-3 sm:p-4 flex flex-col gap-2.5">
                      <!-- Top bar -->
                      <div class="flex items-center gap-2">
                        <div class="min-w-0 flex-1">
                          <p class="text-[8px] text-gray-400 uppercase tracking-wider">
                            {{ copy.portal.welcomeBack }}
                          </p>
                        ...
                        <div class="flex items-center gap-1.5 shrink-0">
                          <UIcon name="i-lucide-search" class="size-3 text-gray-400" />
                          <UIcon name="i-lucide-bell" class="size-3 text-gray-400" />
                          <span
                            class="size-4 rounded-full bg-gradient-to-br text-white text-[7px] font-black flex items-center justify-center"
                            :class="t.grad"
                          >{{ t.logo }}</span>
                        </div>
                      </div>
                      ...
                      <div class="flex-1 grid grid-cols-5 gap-2 min-h-0">
                        <!-- Hero metric card (3 cols) -->
                        <div
                          class="col-span-3 rounded-lg p-2.5 text-white bg-gradient-to-br flex flex-col justify-between"
                          :class="t.grad"
                        >
                          <p class="text-[8px] opacity-80 uppercase tracking-wider">
                            {{ t.heroLabel }}
                          </p>
                          <div>
                            ...
                            <div class="mt-1.5 h-0.5 rounded-full bg-white/25 overflow-hidden">
                              <div
                                class="h-full bg-white/90 rounded-full"
                                :style="{ width: `${t.heroProgress}%` }"
                              />
                            </div>
                          </div>
                        </div>

                        <!-- Next appointment (2 cols) -->
                        <div class="col-span-2 rounded-lg bg-gray-50 dark:bg-white/[0.04] p-2 flex flex-col gap-1">
                          <p class="text-[8px] font-bold text-gray-400 uppercase tracking-wider">
                            {{ copy.portal.nextAppt }}
                          </p>
                          <div class="flex items-center gap-1.5 mt-auto">
                            <UIcon
                              :name="t.apptIcon"
                              class="size-3 shrink-0"
                              :class="t.accent.replace('bg-', 'text-')"
                            />
                            <div class="min-w-0 flex-1">
                      ...
                      <div class="grid grid-cols-3 gap-1.5">
                        <div
                          v-for="q in t.actions"
                          :key="q.label"
                          class="h-8 rounded-md bg-gray-50 dark:bg-white/[0.04] flex items-center justify-center gap-1 px-1.5"
                        >
                          <UIcon
                            :name="q.icon"
                            class="size-3 shrink-0"
                      ...
                      <!-- Bottom pill -->
                      <div class="rounded-full bg-primary text-white flex items-center gap-1.5 px-2 py-1.5">
                        <div
                          class="size-3.5 rounded-full bg-gradient-to-br flex items-center justify-center shrink-0"
                          :class="t.grad"
                        >
                          <UIcon
                            :name="t.pill.icon"
                        ...
                        <p class="text-[9px] font-medium flex-1 truncate">
                          {{ t.pill.text }}
                        </p>
                      </div>
                    </div>
            ...
            <figcaption class="mt-6">
              <div class="flex items-center gap-2 mb-2">
                ...
                <span
                  aria-hidden="true"
                  class="h-px flex-1 bg-black/10 dark:bg-white/10"
                />
                <span class="inline-flex items-center gap-1 text-[10px] uppercase tracking-[0.2em] text-gray-500">
                  <span
                    aria-hidden="true"
              ...
              <h4 class="text-lg sm:text-xl font-black tracking-tight">
                {{ t.name }}
              </h4>
